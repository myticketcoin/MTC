...
#define URI_SCHEME "MyTicketCoin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MTC address (e.g. %1)").arg("M7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no MyTicketCoin: URI
...
{
    // Convert MyTicketCoin:// to MyTicketCoin:
    //
    //    Cannot handle this later, because MyTicketCoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open MyTicketCoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MyTicketCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MyTicketCoin.lnk
...
{
    return GetAutostartDir() / "MyTicketCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a MyTicketCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MyTicketCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the MyTicketCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add MyTicketCoin app to startup item list
